...
EC_Curve::EC_Curve(const BigInteger &prime, const BigInteger &Aval,
                   const BigInteger &Bval) {
...
// EC_Validate - validate the curve for point operations (p prime, discriminant
// not 0)

void EC_Curve::EC_Validate() {
  if ((A * A * A * 4) + (B * B * 27) != 0) {
    if (BI_Miller_Rabin(p))
      valid = true;
    else {
...
bool EC_Curve::EC_Validate_Point(EC_Point pt) {
  ...
  if (pt.zero)
    return true;
  if (pt.x >= p || pt.x < 0 || pt.y >= p || pt.y < 0)
    return false;
...
EC_Point EC_Curve::EC_Add(EC_Point p1, EC_Point p2) {
  ...
  if (!EC_Validate_Point(p1)) {
    std::cerr << "Error(2) : point not on curve - " << std::endl
              << p1.x << std::endl
              << p1.y << std::endl;
...
EC_Point EC_Curve::EC_Double(EC_Point p1) { return EC_Add(p1, p1); }
...
EC_Point EC_Curve::EC_Double_and_Add(EC_Point p1, BigInteger n) {
  if (n <= 0)
    return *(new EC_Point(0));
  vector<int32_t> e = BI_Helper_DecBin(n);
  EC_Point res(0);
...
EC_Point EC_Curve::EC_WindowNAF(EC_Point p1, BigInteger n, int32_t w) {
  if (n <= 0)
    return *(new EC_Point(0));
  if (n <= (1 << w))
    return EC_Double_and_Add(p1, n);
  vector<int32_t> k = BI_Helper_DecBin(n);
  vector<int32_t> naf;
  ...
  // Compute non-adjacent form of the binary representation of n in an integer
  // vector
  ...
  for (i = 0; i <= lim; ++i)
    neg.push_back(EC_Negative(vec[i]));

  // Compute the point multiple

  EC_Point res(0);
  int32_t nsz = naf.size();
  for (i = 0; i < nsz; ++i) {
    res = EC_Double(res);
    if (naf[i] > 0) {
...
void EC_Curve::EC_Find_NAF(int32_t w, vector<int32_t> &k,
                           vector<int32_t> &naf) {
  naf.clear();
  int32_t sz = k.size();
  naf.resize(sz + 1, 0);
